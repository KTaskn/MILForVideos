...
def test_extract():    
    ...
    ds = DataSet(paths_image)
    assert extract(ds, n_batches=1).size() == torch.Size([2, 1024])
    assert extract(ds, n_batches=2).size() == torch.Size([2, 1024])
    
    paths_image = sorted(glob("/datasets/UCSD_Anomaly_Dataset_v1p2/UCSDped1/Train/Train002/*"))[:64]
    ds = DataSet(paths_image)
    assert extract(ds, n_batches=1).size() == torch.Size([4, 1024])
    assert extract(ds, n_batches=2).size() == torch.Size([4, 1024])
